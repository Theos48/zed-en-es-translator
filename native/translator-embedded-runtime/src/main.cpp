...
bool exact_model_path(std::string_view value, std::string_view expected) {
...
std::optional<ModelPaths> parse_model_paths(int argc, char** argv) {
  ...
  for (int index = 1; index < argc; index += 2) {
    ...
    if (flag == "--model" && !model_seen &&
        exact_model_path(value, "models/model.enes.intgemm.alphas.bin")) {
      model_seen = true;
      paths.model = value;
    } else if (flag == "--vocabulary" && !vocabulary_seen &&
               exact_model_path(value, "models/vocab.enes.spm")) {
      vocabulary_seen = true;
      paths.vocabulary = value;
    } else if (flag == "--lexical-shortlist" && !shortlist_seen &&
               exact_model_path(value, "models/lex.50.50.enes.s2t.bin")) {
      shortlist_seen = true;
