...
pub const MANIFEST_DIGEST: &str =
    "d7c9ec7e075ad1ec2158e75a54d71c2422c2cc47efbd79b1308475d874679072";
...
pub fn approved_manifest(runner: &[u8], artifacts: &[FixtureArtifact]) -> String {
    approved_manifest_with_record_version(runner, artifacts, "1")
}

pub fn updated_manifest(runner: &[u8], artifacts: &[FixtureArtifact]) -> String {
    approved_manifest_with_record_version(runner, artifacts, "2")
}

pub fn updated_manifest_digest() -> String {
    let artifacts = fixture_artifacts();
    ProviderManifest::artifact_set_digest_for_review(&updated_manifest(RUNNER, &artifacts))
        .expect("compute updated fixture manifest digest")
}

fn approved_manifest_with_record_version(
    runner: &[u8],
    ...
    let digest = ProviderManifest::artifact_set_digest_for_review(&draft)
        .expect("compute fixture manifest digest");
    draft.replace(MANIFEST_DIGEST, &digest)
}
