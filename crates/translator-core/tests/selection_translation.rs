...
use translator_core::{
    translate_selection_with_confirmation, ErrorCode, InputKind, MockProvider, Provider,
    ProviderRequest, ProviderResponse, TranslateFailure, MAX_INPUT_BYTES,
};
...
fn translates_safe_plain_text_selection_without_mutating_source() {
    ...
    let success = translate_selection_with_confirmation(
        &source,
        InputKind::Text,
        start..end,
        &MockProvider::new(),
...
fn rejects_blank_invalid_and_oversized_selection_ranges() {
    ...
    for range in [0..0, 4..5, 0..source.len() + 1] {
        let error = translate_selection_with_confirmation(
            source,
            InputKind::Text,
            range,
            &MockProvider::new(),
