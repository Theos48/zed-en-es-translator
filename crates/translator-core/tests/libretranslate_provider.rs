...
fn sends_only_permitted_payload_fields() {
    let server = StubHttpServer::new(1, r#"{"translatedText":"Lee la documentacion."}"#);
    ...
    assert_eq!(payload["q"], "Read the docs.");
...
impl StubHttpServer {
    fn new(requests: usize, response_body: &'static str) -> Self {
    ...
    fn first_body(&self) -> String {
        for _ in 0..20 {
            if let Some(body) = self.bodies.lock().expect("bodies lock").first() {
                return body.clone();
            }
            thread::sleep(Duration::from_millis(10));
        }
        panic!("stub server did not receive a request");
    }
}
