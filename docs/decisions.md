...
| D046 | Contrato CLI | El CLI Rust usa JSON UTF-8 por stdin/stdout, una request por proceso, exit code 0 para exito, exit code no cero para error, stderr solo redaccionado y timeout externo. | Aceptada | Mantiene una frontera publica testeable para el core, aunque F005 llame a `translator-core` directamente desde el servidor MCP Rust. |
...
1. Preparar F006 como siguiente feature formal: wrapper Zed para instalar y
   arrancar el servidor MCP sin proveedor real, red ni edicion de buffers.
